...
//console.log("com.sbm.execute : action=>"+sbmObj.action); 
//sbmObj.action = "http://localhost:8080"+sbmObj.action;		//action 도메인 다를경우 처리 불가.... 
//console.log("com.sbm.execute : 변경 action=>"+sbmObj.action); 			
...
var searchCodeGrpOption = {
	 ...
	 submitDoneHandler : scwin.searchCodeGrpCallback, isShowMsg : false };
...
com.sbm.create = function(options) {
	...
	//if (typeof options.isShowMsg !== "undefined") {
		//isShowMsg = options.isShowMsg;
	//}
	
	
	//isShowMsg 프로세스메시지 출력여부 (true, false) => 2021-01-13 LHC 
	if (typeof options.isShowMsg === "undefined"){
		options.isShowMsg = true;
	}
	if (options.isShowMsg === false) {
		processMsg = "";
	} else if ((options.isShowMsg === true) && (processMsg === "")) {
		processMsg = "해당 작업을 처리중입니다";
	}	
	...
	switch (resultData.statusCode) {
		case msgCode.STATUS_ERROR:
			if (resultData.errorCode == "E0001") {
				var alertMsg = com.data.getMessage("MSG_CM_00003", resultData.message);		//{0} 로그인 화면으로 이동하겠습니다.
				com.win.alert(alertMsg, "com.win.goHome");
			} else if (resultData.errorCode == "E9998") { // HTTP ERROR ex) 404,500,0
				resultData.message = resultData.message;
				com.win.alert(resultData.message);
		...
		case "N":
			...
			resultData.message = com.data.getMessage("MSG_CM_00004");
			com.win.alert(resultData.message);
			break;
		default:
			if (com.util.isEmpty(resultData.message) === false) {
				com.win.alert(resultData.message);
			}
	}
...
com.data.setCommonCode = function(codeOptions, callbackFunc) {
	...
	if (codeOptions) {
		...
		$p.log("com.data.setCommonCode Parameter Type Error");
		return;
	}
	 ...
	 * 수행순서
	 * 1. 동적 dataList 생성 
	 * 2. Target 컴포넌트 or 그리드뷰 컬럼에 dataList 바인딩 
	 * 3. 동적 submission 생성
	 * 4. submission 수행 이후 callback 처리 
	 */
	
	var i, j, codeObj, dltId, dltIdArr = [], paramCode = "", compArr, compArrLen, tmpIdArr;
	var dataListOption = _getCodeDataListOptions(gcm.COMMON_CODE_INFO.FILED_ARR);		//[ "GRP_CD", "COM_CD", "CODE_NM" ] 
	...
	for (i = 0; i < codeOptionsLen; i++) {
		...
		try {
			...
			if (typeof $p.top().scwin.commonCodeList[dltId] === "undefined") {
				...
				paramCode += codeObj.code;		//공통코드 키값 
				...
				dataListObj.setJSON(com.util.getJSON($p.top().scwin.commonCodeList[dltId]));
			}
			...
			if (codeObj.compID) {
				...
				for (j = 0; j < compArrLen; j++) {
					...
					//데이터형태(컴포넌트명, 그리드뷰:컬럼명) 에 따라서 구분하여 처리 
					if (tmpIdArr.length === 1) {
						var comp = $p.getComponentById(tmpIdArr[0]);		//데이터형태(컴포넌트명) 일 경우 컴포넌트에 데이터 셋팅 
						comp.setNodeSet("data:" + dltId, gcm.COMMON_CODE_INFO.LABEL, gcm.COMMON_CODE_INFO.VALUE);
					} else {
						var gridObj = $p.getComponentById(tmpIdArr[0]);		//데이터형태(그리드뷰:컬럼명) 일경우 그리드뷰의 컬럼에 데이터 셋팅
	...
	//받아온 데이터 dataList 별로 최상위 배열에 담기 (codeGrpDataListId => dlt_commonCode 형태일 경우 최상위 공통코드배열에 담아두기) 
	searchCodeGrpOption.submitDoneHandler = function(e) {
		for (codeGrpDataListId in e.responseJSON) {
			if (codeGrpDataListId.indexOf(gcm.DATA_PREFIX) > -1) {
				$p.top().scwin.commonCodeList[codeGrpDataListId] = com.str.serialize(e.responseJSON[codeGrpDataListId]);
			}
		}
	...
	//공통코드 키값이 있을경우 submission을 통해서 데이터 조회 
	if (paramCode !== "") {
		com.sbm.create(searchCodeGrpOption);		//submission 동적생성 
		...
		//submission 실행 (해당 sbmObj 로 workflow 사용 시 에는 주석처리할것 )
		//com.sbm.execute(sbmObj);		
...
com.data.getColumnName = function (comObj) {
	try {
		if ((typeof comObj.getRef) === "function") {
			...
			var refArray = ref.substring(5).split(".");		//ref 형태 (data:dma_search.GRP_CODE  이런형태로 data: 를 제외하고 검색) 
			var dataCollectionName = refArray[0];			//dataMap or dataList ID 	
			var columnId = refArray[1];						//컬럼 ID 
		...
		if ((typeof comObj !== "undefined") && (typeof comObj.getRef === "function")) {
			if (comObj.getPluginName() === "gridView") {
				...
				if ((typeof ref !== "undefined") && (ref !== "")) {
					var refArray = ref.substring(5).split(".");				//ref형태 (data:dma_search.GRP_CODE  이런형태로 data: 를 제외하고 검색) 
					if ((typeof refArray !== "undefined") && (refArray.length === 2)) {
						var dataObjInfo = {};
						dataObjInfo.runtimeDataCollectionId = comObj.getScopeWindow().$p.getFrameId() + "_" + refArray[0];
						dataObjInfo.dataColletionId = refArray[0];		//dataMap or dataList ID
						dataObjInfo.columnId = refArray[1];				//컬럼 ID 
						return dataObjInfo;
					} else {
						return null;
					}
 ...
 * @author Inswave Systems
 * @example
var gridObj = grd_AdvancedExcel;
var options = {
	type : "0",
	sheetNo : 0
};
com.data.uploadGridViewExcel(grdObj,  options);
 */
...
var gridObj = grd_AdvancedExcel;
var options = {};
com.data.uploadGridViewCSV(gridObj,  options);
 ...
 */
com.data.createAliasDataList = function( dsId, options ) {
	try {
		var dltObj = com.util.getComponent(dsId);
		if (!com.util.isEmpty(dltObj)) {
			$p.data.remove( dsId );
		...
		_options.studioSrc	= options.studioSrc || "";
 ...
 */
com.data.dataListFilter = function(_dataComp, _condition, _append, options) {
	try {
		...
		if (typeof conditions == "string" && conditions.toUpperCase() == "LIKE") {
			if (options && typeof options.colId != "undefined" && typeof options.func != "undefined") {
				if (typeof filterDs.getColumnIndex(options.colId) == "undefined") {
					com.win.alert("like 사용자지정 필터 컬럼이 존재하지 않습니다.");
				...
				com.win.alert("like 사용자지정 필터 값이 잘못되었습니다.");
			}
		} else { // 일반필터 기능.
			...
			filterDs.setColumnFilter({
				...
				key : function(cellData, tmpParam, rowIdx) {
					try { 
						var conditions = returnConditions.replaceAll( "rowIdx" , "'"+ rowIdx + "'");
						if ( ( new Function( "return " + conditions )() ) ) {
							return true;
						} else {
							return false;
						}
...
com.util.isJSON = function(json) {
	try {
		if (jQuery.isPlainObject(json) === false && com.util.isArray(json) === false) {
			return false;
		}
 ...
 * @update 드릴다운도 적용되도록 수정 드릴다운 페이징은 적용안됨 Inswave Systems
 * @update 전체선택으로 삭제는 rowmoveRows로 처리되도록 수정 Inswave Systems
 * @example
 ...
 */
com.util.clearInterval = function(keyName, force) {
	$p.clearInterval(keyName, force);
};
 ...
 */
com.util.clearTimeout = function(keyName, force) {
	$p.clearTimeout(keyName, force);
};
...
/**
 * 사용자의 권한에 따른 화면 컴포넌트 제어를 한다. (config.xml wframe 로딩시 initScript 에 설정되어 있음) 
 *
 * @private
 * @memberof com.win
 * @date 2020.05.26
...
/**
 * 공통 코드, 권한, 개인화 처리를 위한 Workflow를 실행한다. (config.xml wframe 로딩시 postScript 에 설정되어 있음) 
 *
 * @private
 * @memberof com.win
 * @date 2020.05.27
...
com.win._processCommonData = function() {
	...
	var sbmSearchCode = com.util.getComponent("_sbm_searchCode");		//공통코드 submission ID ( com.data.setCommonCode 함수에서 생성함 ) 
	...
	var sbmSelectShortcutList = com.util.getComponent("_sbm_selectShortcutList");		//단축키 data 조회 submission ID ( com.hkey._setShortKey 함수에서 생성함 ) 
 ...
 * @author Inswave Systems
 * @update Inswave Systems 옵션 파라미터 추가
 * @example
 ...
 * @date 2020.05.16
 * @author Inswave Systems
 * @update Inswave Systems callBackParam , 높이 ,너비 ,url ,className추가
 * @example
//alert창을 띄울 경우
...
com.win._messagBox = function(messageType, messageStr, closeCallbackFncName, opts) {
	...
	var options = {
		id: popId,
		popupName: messageType,	//팝업 title (현재는 messageType(alert/confirm) 으로 표시됨) 
		width: 340,
		height: 150,
		className : "messagebox"
		,disableCloseButton : true		//메시지팝업은 닫기버튼 비활성화 (닫기 시 callback 처리 못하기에..) => .w2window_close_disabled  class 가 추가됨 => all.css 에 추가 
	};
...
com.win._openPopup = function(url, opt, data) {
	...
	var options = {
		...
		scrollbars : true,
		disableCloseButton : opt.disableCloseButton || false,		//닫기버튼 비활성화(true, false) 
		popupUrl : "popup.html"
	};
 ...
 * @author Inswave Systems
 */
com.hkey._setShortKey = function() {
	var param = com.data.getParameter();
	if ((typeof param !== "undefined") && (typeof param.menuCode !== "undefined") && (param.menuCode.trim() !== "")) {
		var menuCd = param.menuCode;
		...
		if (menuInfoList.length > 0) {
			var programList = $p.top().wfm_side.getWindow().dlt_menu.getMatchedJSON("PROGRAM_CD", menuInfoList[0].PROGRAM_CD);
	
			if ((programList.length > 0) || (com.util.isEmpty(programList[0].PROGRAM_CD))) {
				var programCd = programList[0].PROGRAM_CD;			
				var searchCodeGrpOption = { 
					...
					submitDoneHandler : function(e) {
						...
						if (rsMsg.statusCode == gcm.MESSAGE_CODE.STATUS_SUCCESS) {
							...
							for (var i = 0; i < searchShortcutList.length; i++) {
								var isExistRow = $p.top().com.data.dataListFilter("dlt_shortcutList", "PROGRAM_CD == '" + searchShortcutList[i]["PROGRAM_CD"] 
									+ "' && COMPLEX_KEY == '" + searchShortcutList[i]["COMPLEX_KEY"] + "' && LAST_KEY == '" + searchShortcutList[i]["LAST_KEY"] + "'", false);
				...
				var sbmObj = com.util.getComponent("_sbm_selectShortcutList");
				sbmObj.setRequestData(param);
 ...
 * 서버와 Ajax 통신 처리
 * 
 * @description				: 서버와 통신 처리 시 호출 
 * @param <Object:Y> 
 * | options 					JSON형태 객체 
 * | options.action			: [필수] ajax 요청주소(RequestMapping 정보)
 * | options.reqData			: [필수] 요청 데이타 (JSON Object ) 
 * | options.mode         	: sync여부 ( asynchronous(default) || synchronous )
 * | options.isProcessMsg	: 서비스 처리중에 메시지를 보여줄지 여부 설정 ( true(default) || false )
 * | options.processMsg	: 서비스 처리중에 보여줄 메시지 ( 설정하지 않는 경우 기본 메시지를 출력 )
 * | options.resultMsgYn	: 서버에서 전달받은 결과 메시지 Alert 할건지 여부 (true(default) || false )
 * | options.callbackFn		: callback함수
 *  @example
 *  
   			//호출방법 ==============================
   			//요청 데이타 (JSON Object)   			
   			...
   			//통신 처리 
   			var options = {};
   			options.action			= "/prjtm1/test/listData.xml";	//ajax 요청주소 
   			options.reqData		= requestData;					//요청 데이타 
   			options.callbackFn	= "lfn_search_RESULT";			//callback함수명 
   			com.ajax.submit( options );
   			
   			//결과 callback 함수 =======================
   			function lfn_search_RESULT(resBody, e, options){
   				alert("결과 JSON data=>"+JSON.stringify(resBody);
   			}
 */
com.ajax.submit = function( options ){
	try{		
		var action				= options.action;
	    var reqData			= "";
		var mode				= "asynchronous";
		var isProcessMsg		= true;
		var processMsg 		= options.processMsg || "";
		var resultMsgYn		= true;
		var callbackFn			= "";
	    ...
	    if (typeof options.isProcessMsg !== "undefined") {
	    	isProcessMsg = options.isProcessMsg;
	    }
	    if ((isProcessMsg === true) && (processMsg === "")) {
	        processMsg = "loading...";
	    }    
	    if (typeof options.resultMsgYn !== "undefined") {
	    	resultMsgYn = options.resultMsgYn;
	    }    
	    if (typeof options.callbackFn !== "undefined") {
	    	callbackFn = options.callbackFn;
	    }
	    
	    //reqData 에 menuId, PROGRAM_URL 추가 
	    if (typeof options.reqData === "object"){
		    requestData = options.reqData;
		    //requestData.pMenuId	= gfn_menuId();		//메뉴ID 
		    //requestData.pPgmUrl		= PROGRAM_URL;		//프로그램 URL 	    	
		...
		sendOpt.method			= "post";
		sendOpt.mode				= mode; 				//asynchronous || synchronous
		sendOpt.action			= action;
		sendOpt.processMsg		= processMsg;
		sendOpt.requestData	= reqData;
			        		
		sendOpt.success = function (e){
			console.log("e=>"+JSON.stringify(e));	
			console.log("e.responseJSON=>"+JSON.stringify(e.responseJSON));	
			
			resBody = e.responseJSON;
			
			//세션 끊겼을 경우 로그인 페이지로 이동 
			...
			if ( resBody != null ){
				//alert("com.sbm.getResultCode=>"+com.sbm.getResultCode(e));			
				
				/**
				 * Message 처리   "rsMsg":{"message":"권한 리스트가 조회 되었습니다.","statusCode":"S"} 형태 
				 * 		resBody.rsMsg.statusCode    		: 결과 코드(S, E, W)
				 * 		resBody.rsMsg.message       		: 결과 메시지
				 * 		==> rtn_code 가 ERROR일 경우 무조건 message를 Alert 처리 
				 * 		==> rtn_code 가 SUCCESS일 경우 resultMsgYn이 true(defalut)면 rtn_msg를 Alert 처리  
				 */

				if (typeof resBody.rsMsg !== "undefined"){
					if ( (typeof resBody.rsMsg.statusCode !== "undefined") && (resBody.rsMsg.statusCode === gcm.MESSAGE_CODE.STATUS_ERROR) && resultMsgYn ) {
						if ( typeof resBody.rsMsg.message !== "undefined" && resBody.rsMsg.message !== "" ){
							com.sbm.resultMsg(resBody.rsMsg);
						}						
					}
					
					//에러 일 경우 callback 함수 호출 되지 않도록 처리
					if ( (typeof resBody.rsMsg.statusCode !== "undefined") && (resBody.rsMsg.statusCode === gcm.MESSAGE_CODE.STATUS_ERROR) ) {
						return;
					}
				}

				//callback함수 호출 
				if (typeof eval(callbackFn) === "function"){	
					eval(callbackFn)(resBody, e, options);
			    }				
